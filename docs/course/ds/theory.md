...
本讲义为 AI 学组举办的数据结构与算法复习讲座配套资料。内容紧扣考试重点，涵盖复杂度分析（时间复杂度与空间复杂度、渐进记号）、线性表（顺序表与链表及其操作复杂度）、栈与队列（FILO/FIFO 特性、表达式求值、括号匹配）、树与二叉树（遍历、BST、AVL 旋转、哈夫曼编码）、图论基础（存储结构、DFS/BFS、最短路径、最小生成树、拓扑排序）、查找算法（二分查找、哈希表与冲突解决、B/B+ 树索引）以及排序算法（快排/归并/堆排序的对比与稳定性分析）。适合考前快速回顾核心知识点。

> **补充资料**：更多数据结构复习资料可参考 [LeosRealms 复习资料汇总](https://blog.leosrealms.top/tags/review-materials/)。

??? note "数据结构与算法讲座讲义"
    <iframe loading="lazy" src="/static/course/ds/docs/数据结构与算法讲座讲义.pdf" type="application/pdf" width=100% height=1000px></iframe>
...
1. 标记为 $n$ 的函数图像为直线，对应复杂度为 $\mathcal{O}(n)$ ，称为线性阶，表示当问题规模增很大时，运行时间随问题规模 $n$ 线性增加；
...
循环次数为 $100$ ，循环体复杂度为刚刚得到的 $\mathcal{O}(n)$ ，循环复杂度为 $100\mathcal{O}(n)=\mathcal{O}(n)$ 。
...
1. Brute-Force算法：即从第一个字符开始，假设该字符就是模式串的第一个字符，逐个比较接下来的字符，若匹配失败则对下一个字符进行相同的操作，直到匹配成功或遍历完整个串。记模式串和主串长度分别为 $m$ 和 $n$ ，其最坏时间复杂度为 $\mathcal{O}(m\times n)$ （每轮都在模式串最后出现字符不匹配），最佳时间复杂度为 $\mathcal{O}(m)$ （第一次匹配即成功），平均时间复杂度为 $\mathcal{O}(m\times n)$ 。
